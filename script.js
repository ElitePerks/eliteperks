// Define greeting messages in the specified order
const greetings = [
    'Hello,',            // English
    'Hola,',             // Spanish
    'Bonjour,',          // French
...
// Additional text
const additionalText = "I'm Elite Perks and I'm learning Web Development and Ethical Hacking.";

// Function to animate changing greeting
function animateChangeGreeting(greetingElement, newText) {
    let currentIndex = 0;
    let intervalId;

    function typeNextLetter() {
        if (currentIndex <= newText.length) {
            const currentText = newText.substring(0, currentIndex);
    ...
    intervalId = setInterval(typeNextLetter, 200); // Adjust typing speed
}

// Function to change the greeting in the specified order
let index = 0;
function changeGreeting() {
    const greetingElement = document.getElementById('greeting');
    const newGreeting = greetings[index];
    animateChangeGreeting(greetingElement, newGreeting);
    index = (index + 1) % greetings.length; // Move to the next greeting, looping back to the start if necessary
}
